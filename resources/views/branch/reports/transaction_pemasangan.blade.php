    ...
    <div class="row small-spacing justify-content-center">
        <div class="col-12">
            <div class="box-content bordered-all" style="padding: 2% !important;">
                <h4 class="box-title"><i class="fa fa-print"></i> Data Laporan Transaksi Pemasangan By Branch {{ Auth::user()->branch->name }}</h4>
                    ...
                    <tbody>
                        ...
                        @foreach ($transactions as $transaction)
                            <tr>
                                ...
                                <td style="border-top: 1px solid black !important;">
                                    <a href="{{ route('branch.transactions.show', ['type' => 'pemasangan', 'transaction' => $transaction]) }}"  target="__blank">{{ $transaction->nomor_transaction }}</a>
                                </td>
                                <td style="border-top: 1px solid black !important;">
        ...
        $(document).ready(function(){
            ...
            $('.dataTable').dataTable({
                ...
                language: {
                    ...
                    info: "Total <b>_TOTAL_</b> Data <b>Transaksi Pemasangan By Branch {{ Auth::user()->branch->name }}</b> Tanggal : {{ date('d-m-Y', strtotime(request('f_tanggal_start'))) }} S/d. {{ date('d-m-Y', strtotime(request('f_tanggal_end'))) }}",
                    infoEmpty: "Tidak ada data <b>Transaksi Pemasangan By Branch {{ Auth::user()->branch->name }}</b> tersedia",
                }
            });
        });
